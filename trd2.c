...
const char* cheats[7] = {"All Championships", "All Bonus Championships", "Double Power",
                         "Swap FWD and RWD", "Invincible Cars", "All Cutscenes", "All Trans World Cup Events"};

const char* platforms[4] = {"PC", "PS2", "PSP", "XBOX"};

int isplatform(char platform[]) {
    for(size_t i = 0; i < strlen(platform); i++)
...
int32_t calcseed(const int32_t input, const int32_t mult, const int32_t array[]) {
    ...
    if(input != 0) {
        ...
        for(int32_t i = 0; i < input - 1; i++)
            seed = ((array[2] + seed * array[1] * mult) & 0xFFFFFFFF) % array[3];
    }
...
char* gencodeps2(const int32_t accesscode, const int32_t cheatid) {
    ...
    while(loopctr != 0) {
        if(loopctr & 1)
            val1 = (((((((val1 * val2) % 0xFFFFFFFB) + ((((val1 >> 0x20) * val2) % 0xFFFFFFFB) * 0x40)) % 0xFFFFFFFB) + (((val1 * (val2 >> 0x20)) % 0xFFFFFFFB) * 0x40)) % 0xFFFFFFFB) + ((((val1 >> 0x20) * (val2 >> 0x20)) % 0xFFFFFFFB) * 0x80)) % 0xFFFFFFFB;
    ...
    static char code[9];
    for(int i = 0; i < 8; i++)
        code[i] = 0x41 + ((seed / pwr(0x1A, i)) % 0x1A);
...
char* gencode(const char platform[], const int32_t accesscode, const int32_t cheatid) {
    if(!strcmp(platform, "PS2"))
        return gencodeps2(accesscode, cheatid);

    int32_t array[4];
    if(!strcmp(platform, "PC"))
        memcpy(array, (int32_t[4]) {79838997, 107, 453889, 456959}, 4 * sizeof(int32_t));
    else if(!strcmp(platform, "PSP"))
        memcpy(array, (int32_t[4]) {31873457, 211, 453889, 456959}, 4 * sizeof(int32_t));
    else if(!strcmp(platform, "XBOX"))
        memcpy(array, (int32_t[4]) {93199323, 179, 453889, 456959}, 4 * sizeof(int32_t));
...
int main(int argc, char* argv[]) {
    ...
    for(int i = 1; i < 3; i++) {
        if(isplatform(argv[i]))
            ...
            accesscode = strtol(argv[i], NULL, 10);
    }
